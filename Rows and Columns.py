...
external_stylesheets = ['assets/diaperstyles.css', dbc.themes.BOOTSTRAP]
    ...
    children=[
        ...
        html.Div(
            children=[
                ...
                html.Div([
                    dbc.Row([
                        dbc.Col(dcc.Graph(id='preterm-content'), width=6),
                        dbc.Col(dcc.Graph(id='childcare1-content'), width=6),
                    ]),
                    dbc.Row([
                        dbc.Col(dcc.Graph(id='childcare2-content'), width=6),
                        dbc.Col(dcc.Graph(id='income2019-content'), width=6),
                    ]),
                    dbc.Row([
                        dbc.Col(dcc.Graph(id='income2020-content'), width=6),
                        dbc.Col(dcc.Graph(id='illness-content'), width=6),
                    ]),
                    dbc.Row([
                        dbc.Col(dcc.Graph(id='transport-content'), width=6),
                        dbc.Col(dcc.Graph(id='transport-pie-content'), width=6),
                    ]),
                    dbc.Row([
                        dbc.Col(dcc.Graph(id='DR-content'), width=6),
                        dbc.Col(dcc.Graph(id='SevDR-content'), width=6),
                    ]),
                    dbc.Row([
                        dbc.Col(dcc.Graph(id='UTI-content'), width=6),
                        dbc.Col(dcc.Graph(id='Education-content'), width=6),
                    ]),
                    #children=[
...
def update_preterm(race, state, singlehead):
    ...
    dff = dff.replace(2, 0)
    dff['SumPreterm'] = dff[
        ['CH1Preterm', 'CH2Preterm', 'CH3Preterm', 'CH4Preterm', 'CH5Preterm', 'CH6Preterm', 'CH7Preterm','CH8Preterm']].sum(axis=1)
...
def update_illness1(race, state, singlehead):
    ...
    if 'race' in filters or 'state' in filters or 'singlehead' in filters:
        dff = df[
            ['Race', 'State', 'Single Household', 'CH1HaveRashBefore', 'CH1HaveRashAfter','CH2HaveRashBefore', 'CH2HaveRashAfter']]
        if filters['race']:
            dff = dff.loc[dff['Race'] == filters['race']]
        if filters['state']:
            dff = dff.loc[dff['State'] == filters['state']]
...
def update_illness1(race, state, singlehead):
    ...
    if 'race' in filters or 'state' in filters or 'singlehead' in filters:
        dff = df[
            ['Race', 'State', 'Single Household', 'CH1HaveSevRashBefore', 'CH1HaveSevRashAfter','CH2HaveSevRashBefore', 'CH2HaveSevRashAfter']]
        if filters['race']:
            dff = dff.loc[dff['Race'] == filters['race']]
        if filters['state']:
            dff = dff.loc[dff['State'] == filters['state']]
...
def update_illness2(race, state, singlehead):
    ...
    if 'race' in filters or 'state' in filters or 'singlehead' in filters:
        dff = df[['Race', 'State', 'Single Household', 'CH1HaveUTIBefore', 'CH1HaveUTIAfter','CH2HaveUTIBefore', 'CH2HaveUTIAfter']]
        if filters['race']:
            dff = dff.loc[dff['Race'] == filters['race']]
        if filters['state']:
            dff = dff.loc[dff['State'] == filters['state']]
